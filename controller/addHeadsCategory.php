<?php 
	...
	if(isset($_POST) && $_POST['flag'] == "addHeadsCategory"){
		...
		if(isset($_FILES["head_picture"]["name"]) && $_FILES["head_picture"]["name"] != ""){
			...
			if (move_uploaded_file($_FILES["head_picture"]["tmp_name"], $target_file)) {
				$coloum = "`head_id`,`name`, `description`, `head_picture`, `date`, `status`, `admin_ids`";
				$data = "'".$head_id."','".$name."','".$description."','".$head_picture."','".$date."','1','".$admin_ids."'";
			
				$record = $ob->commonInsert("heads_category",$coloum,$data);
			...
			$coloum = "`head_id`,`name`, `description`, `date`, `status`, `admin_ids`";
			$data = "'".$head_id."','".$name."','".$description."','".$date."','1','".$admin_ids."'";
